...
import { Server, CheckCircle2, ShieldCheck, RefreshCw, Network } from "lucide-react";
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white font-outfit">Active Directory Integration</h1>
          <p className="mt-2 text-slate-600 dark:text-slate-400">Manage on-premise AD synchronization via LDAP.</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
              ...
              <form className="space-y-6">
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">AD Server IP / Hostname</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Port Number</label>
                    <input 
                      type="number" 
                      defaultValue="389"
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/5 rounded-lg px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-600 dark:focus:ring-indigo-500 transition-all outline-none"
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Domain Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Base DN</label>
                    <input 
                      type="text" 
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Bind Username</label>
                  <input 
                    type="text" 
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/5 rounded-lg px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-600 dark:focus:ring-indigo-500 transition-all outline-none"
                  />
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Bind Password</label>
                  <input 
                    type="password" 
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/5 rounded-lg px-4 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-600 dark:focus:ring-indigo-500 transition-all outline-none"
                  />
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-2">
                    <input type="checkbox" className="w-4 h-4 rounded bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-white/10 text-indigo-600 focus:ring-indigo-500" />
                    <label className="text-sm text-slate-600 dark:text-slate-300">Enable LDAPS (SSL)</label>
                  </div>
                </div>

                <div className="flex items-center gap-4 pt-4">
                  <button type="button" className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2.5 rounded-lg font-semibold transition-all shadow-lg shadow-indigo-500/20">
                    Save Config
                  </button>
                  <button 
                    type="button"
                  ...
                  >
                    {testing ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Server className="h-4 w-4" />}
                    Test Connection
                  </button>
                </div>
              </form>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-500/20 rounded-2xl p-6 text-indigo-600/80 dark:text-indigo-200/70 text-sm">
              ...
              Always use a dedicated Service Account with "Read-only" permissions for basic sync, or "Account Operator" for provisioning.
            </div>
          </div>
        </div>
      </div>
