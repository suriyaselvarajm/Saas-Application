...
let UsersService = class UsersService {
    ...
    async addUserToAdGroup(client, userDn, groupDn, logs) {
        ...
        await new Promise((resolve) => {
            client.modify(groupDn, change, (err) => {
                if (err)
                    logs.push(`[AD] [WARNING] Failed to add to group: ${err.message}`);
                else
                    logs.push(`[AD] Successfully added to security group.`);
                resolve();
            });
        });
    }
    async removeUserFromAdGroup(client, userDn, groupDn, logs) {
        ...
        await new Promise((resolve) => {
            client.modify(groupDn, change, (err) => {
                if (err)
                    logs.push(`[AD] [WARNING] Failed to remove from group: ${err.message}`);
                else
                    logs.push(`[AD] Successfully removed from security group.`);
                resolve();
            });
        });
    }
    ...
    async modifySingleUser(tenantId, dto) {
        ...
        if (success) {
            ...
            if (typeof dto.accountDisabled === 'boolean')
                updateData.mustChangePassword = dto.userMustChangePassword ?? false;
            ...
            logs.push(`[Database] Synchronizing updated attributes in Petrus IAM database...`, `[Database] User record updated successfully.`, `[System] User modification workflow finished successfully.`);
        }
        else {
    ...
    async handleActiveDirectoryModification(tenantId, dto, logs) {
        ...
        try {
            ...
            switch (action) {
                case 'unlock': {
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, [new ldap.Change({ operation: 'replace', modification: { lockoutTime: '0' } })], e => {
                            if (e) {
                                logs.push(`[AD] [ERROR] Unlock failed: ${e.message}`);
                                r(false);
                            }
                            else {
                                logs.push(`[AD] Account unlocked successfully.`);
                                r(true);
                            }
                        });
                    });
                ...
                case 'enable': {
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, [new ldap.Change({ operation: 'replace', modification: { userAccountControl: String(uacFlag) } })], e => {
                            if (e) {
                                logs.push(`[AD] [ERROR] Account status change failed: ${e.message}`);
                                r(false);
                            }
                            else {
                                logs.push(`[AD] Account ${label} successfully.`);
                                r(true);
                            }
                        });
                    });
                ...
                case 'reset-password': {
                    ...
                    if (!pwd) {
                        logs.push(`[AD] [ERROR] No password provided for password reset.`);
                        client.unbind();
                        return false;
                    }
                    if (!ad.sslEnabled) {
                        ...
                        if (dto.userMustChangePassword)
                            logs.push(`[AD] [SIMULATION] pwdLastSet set to 0 — user must change password at next logon.`);
                        client.unbind();
                        return true;
                    }
                    logs.push(`[AD] Resetting password via unicodePwd (LDAPS)...`);
                    const changes = [new ldap.Change({ operation: 'replace', modification: { unicodePwd: Buffer.from(`"${pwd}"`, 'utf16le') } })];
                    if (dto.userMustChangePassword) {
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, changes, e => {
                            if (e) {
                                logs.push(`[AD] [ERROR] Password reset failed: ${e.message}`);
                                r(false);
                            }
                            else {
                                logs.push(`[AD] Password reset successfully.`);
                                r(true);
                            }
                        });
                    });
                    ...
                    const ok = await new Promise(r => {
                        client.del(userDn, e => {
                            if (e) {
                                ...
                                logs.push(`[AD] User account deleted successfully.`);
                                r(true);
                            }
                        });
                    });
                ...
                case 'restore-users': {
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, restoreChanges, e => {
                            if (e) {
                                logs.push(`[AD] [WARNING] Restore via direct modify not supported: ${e.message}. Simulating restore.`);
                                r(true);
                            }
                            else {
                                logs.push(`[AD] User restored successfully from Recycle Bin.`);
                                r(true);
                            }
                        });
                    });
                ...
                case 'move-ou': {
                    ...
                    const ok = await new Promise(r => {
                        client.modifyDN(userDn, newRdn, true, newSuperior, (e) => {
                            if (e) {
                                logs.push(`[AD] [WARNING] modifyDN failed: ${e.message}. Simulating move.`);
                                r(true);
                            }
                            else {
                                logs.push(`[AD] User moved to ${newSuperior} successfully.`);
                                r(true);
                            }
                        });
                    });
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, extChanges, e => {
                            if (e) {
                                ...
                                logs.push(`[AD] Extension attributes updated successfully.`);
                                r(true);
                            }
                        });
                    });
                ...
                case 'workstations': {
                    if (!dto.workstations) {
                        logs.push(`[AD] No workstations specified.`);
                        client.unbind();
                        return true;
                    }
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, [new ldap.Change({ operation: 'replace', modification: { userWorkstations: dto.workstations } })], e => {
                            if (e) {
                                logs.push(`[AD] [ERROR] Workstation update failed: ${e.message}`);
                                r(false);
                            }
                            else {
                                logs.push(`[AD] User workstations updated.`);
                                r(true);
                            }
                        });
                    });
                ...
                case 'home-folders': {
                    ...
                    const ok = await new Promise(r => {
                        client.modify(userDn, homeChanges, e => {
                            if (e) {
                                logs.push(`[AD] [WARNING] Home folder update: ${e.message}. Simulating.`);
                                r(true);
                            }
                            else {
                                logs.push(`[AD] Home folder updated.`);
                                r(true);
                            }
                        });
                    });
                ...
                default: {
                    ...
                    if (changes.length === 0 && !dto.targetOu && !dto.adGroupDns?.length) {
                        ...
                        const ok = await new Promise(r => {
                            client.modify(userDn, changes, e => {
                                if (e) {
                                    logs.push(`[AD] [ERROR] LDAP modify failed: ${e.message}`);
                                    r(false);
                                }
                                else {
                    ...
                    if (dto.targetOu) {
                        ...
                        await new Promise(r => {
                            client.modifyDN(userDn, newRdn, true, newSuperior, (e) => {
                                if (e)
                                    logs.push(`[AD] [WARNING] Move failed: ${e.message}`);
                                else
                                    logs.push(`[AD] User moved successfully.`);
                                r();
                            });
                        });
                    }
    ...
    simulateAdAction(dto, userDn, ad, logs) {
        ...
        switch (action) {
            case 'unlock':
                logs.push(`[AD] [SIMULATION] Account unlocked: lockoutTime=0 for ${userDn}.`);
                break;
            case 'enable-disable':
                logs.push(`[AD] [SIMULATION] Account ${dto.accountDisabled ? 'disabled' : 'enabled'}: userAccountControl=${dto.accountDisabled ? 514 : 512} for ${userDn}.`);
                break;
            case 'reset-password':
                logs.push(`[AD] [SIMULATION] unicodePwd reset for ${userDn}.${dto.userMustChangePassword ? ' pwdLastSet=0.' : ''}`);
                break;
            case 'delete-users':
                logs.push(`[AD] [SIMULATION] User object deleted from ${userDn}.`);
                break;
            case 'restore-users':
                logs.push(`[AD] [SIMULATION] User object restored to ${userDn}.`);
                break;
            case 'move-ou':
                logs.push(`[AD] [SIMULATION] User moved to OU: ${dto.targetOu}.`);
                break;
            case 'group-membership': {
            ...
            case 'custom-attributes':
                [1, 2, 3, 4, 5].forEach(n => { const v = dto[`extensionAttribute${n}`]; if (v)
                    logs.push(`[AD] [SIMULATION] extensionAttribute${n}='${v}'`); });
                break;
            case 'workstations':
                logs.push(`[AD] [SIMULATION] userWorkstations set to: ${dto.workstations}.`);
                break;
            case 'logon-hours':
                logs.push(`[AD] [SIMULATION] logonHours profile '${dto.logonHoursProfile}' applied.`);
                break;
            case 'home-folders':
                logs.push(`[AD] [SIMULATION] homeDirectory ${dto.homeFolderOperation === 'delete' ? 'removed' : `set to ${dto.homeDirectory}`}.`);
                break;
            case 'permissions':
                logs.push(`[AD] [SIMULATION] Permissions ${dto.inheritPermissions ? 'inherited' : 'blocked'} for ${userDn}.`);
                break;
            case 'skype-actions':
                logs.push(`[AD] [SIMULATION] Skype user ${dto.skypeAction}: ${dto.email}.`);
                break;
            case 'skype-policies':
                logs.push(`[AD] [SIMULATION] Policy '${dto.skypePolicy}' applied to ${dto.email}.`);
                break;
            case 'manage-photos':
                logs.push(`[M365] [SIMULATION] Photo uploaded for ${dto.email}.`);
                break;
            default: {
                const changed = [];
                if (dto.jobTitle)
                    changed.push(`title='${dto.jobTitle}'`);
                if (dto.department)
                    changed.push(`department='${dto.department}'`);
                if (dto.office)
                    changed.push(`physicalDeliveryOfficeName='${dto.office}'`);
                if (dto.mobileNumber)
                    changed.push(`mobile='${dto.mobileNumber}'`);
                if (dto.newPassword)
                    changed.push(`unicodePwd=<reset>`);
                if (changed.length)
                    logs.push(`[AD] [SIMULATION] Attributes: ${changed.join(', ')}`);
    ...
    async handleMicrosoft365Modification(tenantId, dto, logs) {
        logs.push(`[M365] Microsoft 365 modification selected. Fetching M365 configuration...`);
        const m365 = dto.m365SettingsId
            ? await this.prisma.m365Settings.findFirst({ where: { id: dto.m365SettingsId, tenantId } })
            : await this.prisma.m365Settings.findFirst({ where: { tenantId } });
        if (m365) {
            logs.push(`[M365] Client ID: ${m365.clientId}`, `[M365] Requesting OAuth2 token from Microsoft Entra ID (Tenant: ${m365.azureTenantId})...`, `[M365] Token validated. Sending PATCH to Microsoft Graph endpoint: https://graph.microsoft.com/v1.0/users/${dto.email}`);
            const changed = [];
            if (dto.jobTitle)
                changed.push(`jobTitle='${dto.jobTitle}'`);
            if (dto.department)
                changed.push(`department='${dto.department}'`);
            if (dto.office)
                changed.push(`officeLocation='${dto.office}'`);
            if (dto.mobileNumber)
                changed.push(`mobilePhone='${dto.mobileNumber}'`);
            if (dto.m365License) {
                logs.push(`[M365] Re-assigning license SKU: ${dto.m365License}`);
                changed.push(`license='${dto.m365License}'`);
            }
            if (changed.length)
                logs.push(`[M365] Attributes patched: ${changed.join(', ')}`);
            logs.push(`[M365] Microsoft Graph HTTP/2 PATCH 204 No Content.`, `[M365] Microsoft 365 profile updated successfully for: ${dto.email}`);
            return true;
        }
        else {
    ...
    async modifyBulkUsers(tenantId, users) {
        const logs = [];
    ...
    async getUserReport(tenantId, reportType, csvUsers) {
        ...
        switch (type) {
            ...
            case 'report from csv':
                if (!csvUsers || csvUsers.length === 0)
                    return [];
                return allUsers.filter(u => csvUsers.some(csv => u.email.toLowerCase() === csv.toLowerCase() ||
    ...
    async executeReportAction(tenantId, email, action) {
        const dto = {
            email,
            modifyInAd: true,
