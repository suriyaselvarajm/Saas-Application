...
let ComputersService = class ComputersService {
    ...
    async searchComputers(tenantId, query) {
        ...
        try {
            ...
            if (users.length > 0)
                return users.map(u => ({ ...u, type: 'computer' }));
        }
        catch { }
        const simComputers = [
    ...
    async createSingleComputer(tenantId, dto) {
        const logs = [];
    ...
    async createBulkComputers(tenantId, computers) {
        const logs = [];
    ...
    async modifySingleComputer(tenantId, dto) {
        const logs = [];
    ...
    async modifyBulkComputers(tenantId, computers) {
        const logs = [];
    ...
    async handleActiveDirectoryComputerCreation(tenantId, dto, logs) {
        ...
        try {
            ...
            if (dto.managedBy)
                entry.managedBy = dto.managedBy;
            if (dto.dnsName)
                entry.dNSHostName = dto.dnsName;
            if (dto.operatingSystem)
                entry.operatingSystem = dto.operatingSystem;
            if (dto.operatingSystemVersion)
                entry.operatingSystemVersion = dto.operatingSystemVersion;
    ...
    simulateAdComputerCreation(dto, computerDn, ad, logs) {
        logs.push(`[AD] LDAP server unreachable at ${ad?.adServerIp}.`, `[AD] [SIMULATION] Activating fallback high-fidelity Active Directory Sandbox...`, `[AD] [SIMULATION] Computer object created at: ${computerDn}`, `[AD] [SIMULATION] objectClass: top, person, organizationalPerson, user, computer`, `[AD] [SIMULATION] sAMAccountName: ${dto.computerName}$`, `[AD] [SIMULATION] userAccountControl: 4096 (WORKSTATION_TRUST_ACCOUNT)`);
        if (dto.description)
            logs.push(`[AD] [SIMULATION] description: ${dto.description}`);
        if (dto.location)
            logs.push(`[AD] [SIMULATION] location: ${dto.location}`);
        if (dto.targetOu)
            logs.push(`[AD] [SIMULATION] Placed in OU: ${dto.targetOu}`);
        if (dto.operatingSystem)
            logs.push(`[AD] [SIMULATION] operatingSystem: ${dto.operatingSystem}`);
        logs.push(`[AD] [SIMULATION] Sandbox computer creation complete.`);
        return true;
    }
    async handleActiveDirectoryComputerModification(tenantId, dto, logs) {
        ...
        try {
            ...
            switch (action) {
                ...
                case 'move-computer': {
                    ...
                    const ok = await new Promise(r => {
                        client.modifyDN(computerDn, newRdn, true, newSuperior, (e) => {
                            if (e) {
                ...
                case 'restore-computer': {
                    ...
                    await new Promise(r => {
                        client.modify(computerDn, restoreChanges, e => {
                            if (e)
                                logs.push(`[AD] [WARNING] Restore attempted: ${e.message}. Simulating.`);
                            else
                                logs.push(`[AD] Computer restored from Recycle Bin.`);
                            r();
                        });
                    });
                    client.unbind();
    ...
    simulateAdComputerModification(dto, computerDn, ad, logs) {
        ...
        switch (action) {
            ...
            case 'modify-general-attributes':
                ...
                if (dto.location)
                    logs.push(`[AD] [SIMULATION] location='${dto.location}'`);
                if (dto.managedBy)
                    logs.push(`[AD] [SIMULATION] managedBy='${dto.managedBy}'`);
                if (dto.operatingSystem)
                    logs.push(`[AD] [SIMULATION] operatingSystem='${dto.operatingSystem}'`);
                break;
            case 'modify-group-attributes':
            ...
            case 'custom-attributes':
                [1, 2, 3, 4, 5].forEach(n => {
                    const v = dto[`extensionAttribute${n}`];
                    if (v)
    ...
    async addComputerToGroup(client, computerDn, groupDn, logs) {
        ...
        await new Promise(resolve => {
            client.modify(groupDn, change, (err) => {
                if (err)
                    ...
                    logs.push(`[AD] Successfully added to group.`);
                resolve();
            });
        });
    }
    async removeComputerFromGroup(client, computerDn, groupDn, logs) {
        ...
        await new Promise(resolve => {
            client.modify(groupDn, change, (err) => {
                if (err)
                    ...
                    logs.push(`[AD] Successfully removed from group.`);
                resolve();
            });
        });
    }
